...
public class SecurityConfig {
    ...
    public InMemoryUserDetailsManager inMemoryUserDetailsManager(PasswordEncoder passwordEncoder) {
        return new InMemoryUserDetailsManager(
                ...
                User.withUsername("ahmed").password(passwordEncoder.encode("1234")).roles("USER","AGENT").build(),
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
        ...
        httpSecurity.authorizeRequests().requestMatchers("/saveBus").hasAnyRole("ADMIN","AGENT");
        httpSecurity.authorizeRequests().requestMatchers("/listeBus") .hasAnyRole("ADMIN","AGENT","USER");
        httpSecurity.authorizeRequests().requestMatchers("/supprimerBus","/editerBus","/updateProduit") .hasAnyRole("ADMIN");
